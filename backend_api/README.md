...
* [Регистрация нового пользователя](#create_recipe)

## Работа с рецептами
* [Создание нового рецепта](#register)
...
Для создания нового рецепта необходимо сделать запрос `POST http://127.0.0.1:8000/api/recipes`  
Список ключей при создании рецепта (формат запроса - form-data)  

+ name: Название рецепта
+ description: Описание рецепта (максимум 1500 знаков)
+ category: Категория1, Категория2
+ cooking_instructions: Инструкция приготовления (максимум 1500 знаков)
+ cooking_time_in_minutes: Время приготовления в минутах (просто число)
+ image: Изображение

При создании рецепта нужно выбрать изображение в формате jpeg, jpg или png, размером до 2мб и прикрепить файл по ключу `image`.  
По ключу `category` можно отправить несколько категорий, указанных через запятую. Если такой категории в базе данных еще нет, она создастся автоматически.  
Доступны методы: `GET, POST, PUT, DELETE`
Во всех методах, кроме `GET` требуется токен в заголовке авторизации.
